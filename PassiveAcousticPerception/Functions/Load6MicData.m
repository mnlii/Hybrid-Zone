...
colorbar;
    ...
    raw_data(1:xcorrNum*SignalParam.ChirpSize,:)=[]; % delete 10 chirps i.e. 0.4s
    AlignIdx1=zeros(6,1);
    for ii=1:6
        subplot(6,3,3*ii-1);
    ...
    raw_data(1:mod(round(mean(rmoutliers(AlignIdx1))+mean(rmoutliers(AlignIdx2))),SignalParam.ChirpSize),:)=[]; %minor deletion, delete < 1 chirp, i.e. 0.04s
